...
import tokens = require('./tokenController')
...
interface AuthSelf {
  ...
  pass: { min: number; max: number; rand: number }
  assertUser: (token: string, fields?: string | string[], ip?: string) => Promise<any>
  requireUser: (req: any, res: any, next: (error?: any) => void, options?: any) => void
  optionalUser: (req: any, res: any, next: (error?: any) => void, options?: any) => void
  verify: (req: any, res: any) => Promise<any>
  register: (req: any, res: any) => Promise<any>
  changePassword: (req: any, res: any) => Promise<any>
  assertPermission: (user: any, target: any) => void
  createUser: (req: any, res: any) => Promise<any>
  editUser: (req: any, res: any) => Promise<any>
  disableUser: (req: any, res: any) => Promise<any>
  deleteUser: (req: any, res: any) => Promise<any>
  bulkDeleteUsers: (req: any, res: any) => Promise<any>
  listUsers: (req: any, res: any) => Promise<any>
}
...
self.assertUser = async (token: string, fields?: string | string[], ip?: string): Promise<any> => {
  if (ip) {
...
self.requireUser = (req: any, res: any, next: (error?: any) => void, options: any = {}): void => {
...
self.optionalUser = (req: any, res: any, next: (error?: any) => void, options: any = {}): void => {
  const token = options.token || req.headers.token
  if (!token) {
...
self.register = async (req: any, res: any): Promise<any> => {
...
self.changePassword = async (req: any, res: any): Promise<any> => {
  const password = typeof req.body.password === 'string'
    ? req.body.password.trim()
    : ''
  if (password.length < self.pass.min || password.length > self.pass.max) {
...
self.editUser = async (req: any, res: any): Promise<any> => {
  const isadmin = perms.is(req.locals.user, 'admin')
  if (!isadmin) {
    return res.status(403).end()
  }
  ...
  const response: any = { success: true, update }
  if (password) {
...
self.disableUser = async (req: any, res: any): Promise<any> => {
  req.body = {
    id: req.body.id,
    enabled: false
  }
...
self.deleteUser = async (req: any, res: any): Promise<any> => {
  const isadmin = perms.is(req.locals.user, 'admin')
  if (!isadmin) {
    return res.status(403).end()
  }
...
self.bulkDeleteUsers = async (req: any, res: any): Promise<any> => {
  // TODO
}

self.listUsers = async (req: any, res: any): Promise<any> => {
  const isadmin = perms.is(req.locals.user, 'admin')
  if (!isadmin) {
    return res.status(403).end()
  }

  const result: any = { success: true, users: [], usersPerPage, count: 0 }
