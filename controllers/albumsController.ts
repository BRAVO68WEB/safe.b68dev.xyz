...
import ServeStatic = require('./handlers/ServeStatic')
...
interface AlbumsSelf {
  titleMaxLength: number
  descMaxLength: number
  onHold: Set<string>
  zipEmitters: Map<string, any>
  getUniqueAlbumIdentifier: (res?: any) => Promise<string>
  unholdAlbumIdentifiers: (res: any) => void
  list: (req: any, res: any) => Promise<any>
  create: (req: any, res: any) => Promise<any>
  delete: (req: any, res: any) => Promise<any>
  disable: (req: any, res: any) => Promise<any>
  edit: (req: any, res: any) => Promise<any>
  rename: (req: any, res: any) => Promise<any>
  get: (req: any, res: any) => Promise<any>
  getUpstreamCompat: (req: any, res: any) => Promise<any>
  generateZip: (req: any, res: any) => Promise<any>
  addFiles: (req: any, res: any) => Promise<any>
}

const self: AlbumsSelf = {
  titleMaxLength: 70,
  descMaxLength: 4000,
...
const zipOptions: any = config.uploads.jsZipOptions || {}
...
self.getUniqueAlbumIdentifier = async (res?: any): Promise<string> => {
...
self.unholdAlbumIdentifiers = (res: any): void => {
...
self.list = async (req: any, res: any): Promise<any> => {
  ...
  const result: any = { success: true, albums: [], albumsPerPage, count: 0, homeDomain }
  ...
  const albumids: Record<number, any> = {}
...
self.create = async (req: any, res: any): Promise<any> => {
...
self.disable = async (req: any, res: any): Promise<any> => {
  const ismoderator = perms.is(req.locals.user, 'moderator')
...
self.edit = async (req: any, res: any): Promise<any> => {
  const ismoderator = perms.is(req.locals.user, 'moderator')
...
self.rename = async (req: any, res: any): Promise<any> => {
  req.body = {
    _legacy: true,
    name: req.body.name
  }
...
self.getUpstreamCompat = async (req: any, res: any): Promise<any> => {
...
self.generateZip = async (req: any, res: any): Promise<any> => {
  ...
  if (album.zipGeneratedAt > album.editedAt) {
    try {
      ...
      return serveAlbumZipInstance.handle(req, res, filePath, stat, (req: any, res: any) => {
  ...
  if (self.zipEmitters.has(identifier)) {
    return new Promise<any>((resolve, reject) => {
      ...
      self.zipEmitters.get(identifier).once('done', (result: any, clientErr: any) => {
        if (clientErr || !result) {
          return reject(clientErr || new ServerError('Album ZIP generation failed.'))
        }
        return resolve(result)
      })
    }).then(async (result: any) =>
      serveAlbumZipInstance.handle(req, res, result.path, result.stat, (req: any, res: any) => {
  ...
  if (zipMaxTotalSize) {
    const totalSizeBytes = files.reduce((accumulator: number, file: any) => accumulator + parseInt(file.size), 0)
  ...
  return serveAlbumZipInstance.handle(req, res, result.path, result.stat, (req: any, res: any) => {
    res.header('Content-Disposition', contentDisposition(result.name, { type: 'inline' }))
  })
}

self.addFiles = async (req: any, res: any): Promise<any> => {
  ...
  return Promise.resolve().then(async () => {
    ...
    failed.push(...ids.filter((id: any) => !files.find((file: any) => file.id === id)))

    await utils.db.transaction(async (trx: any) => {
      await trx('files')
        .whereIn('id', files.map((file: any) => file.id))
